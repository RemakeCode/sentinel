...
// updateWatchlistFile updates the watchlist.jsonc file with currently watched paths
func (s *Service) updateWatchlistFile() {
	if s.watcher == nil {
		return
	}

	watchedPaths := s.watcher.WatchList()

	// Create JSON content
	jsonData, err := json.MarshalIndent(watchedPaths, "", "  ")
	if err != nil {
		slog.Error("Failed to marshal watchlist", "error", err)
		return
	}

	// Prepare file content with comment
	content := fmt.Sprintf("// This file is autogenerated and managed by Sentinel. Do not edit\n%s", string(jsonData))

	watchlistPath := filepath.Join(backend.ConfigDir, "watchlist.jsonc")
	err = os.WriteFile(watchlistPath, []byte(content), 0644)
	if err != nil {
		slog.Error("Failed to write watchlist file", "path", watchlistPath, "error", err)
	} else {
		slog.Debug("Watchlist file updated", "path", watchlistPath)
	}
}
